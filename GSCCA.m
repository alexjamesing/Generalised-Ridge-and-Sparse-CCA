function [c_arr,w_arr,Corr_mat] = GSCCA(X_arr, Spar_arr,w_sgn_arr,C_mv,niter)
...
n  
    ...
    for j = 1:size(X_arr,2); %% in this loop, we apply the msCCA algorithm in each view of the data
        ...
        if size(X_arr{1,j},2)~=1   
         ...
         a = scca_vec(X_arr,w_arr,C_mv,j); %% here, we calculate scca_vectors on the basis of the scca_vec subroutine 
...
mean_corr_between_sets = sum(sum(Corr_mat(find(C_mv))))/sum(sum(C_mv))  %#ok<NOPRT,NASGU>
end

end

function a = scca_vec(X_arr,w_arr,C_mv,j) %% this sub-routine is used to calculate the correlation between sets 
